export class HUD {
  ...
  update() {
    ...
    this.debug.innerHTML = `state: ${g.ball.state}<br>raw: ${g.ball.x.toFixed(1)},${g.ball.y.toFixed(1)}<br>out: ${outFlags}<br>possessor: ${g.ball.carrier ? g.ball.carrier.name : 'None'}<br>ball: ${Math.round(g.ball.x)},${Math.round(g.ball.y)}<br>v: ${Math.round(g.ball.vx)},${Math.round(g.ball.vy)} speed:${Math.round(g.ball.speed)}<br>target: ${g.ball.target ? `${Math.round(g.ball.target.x)},${Math.round(g.ball.target.y)}` : 'None'}<br>pass: ${g.ball.lastPassType}<br>shot src: ${g.lastShotSource}<br>last touch: ${g.ball.lastTouchTeam?.name || 'None'} / ${g.ball.lastTouch?.name || 'None'}<br>boundary: ${g.lastBoundary || 'none'}<br>restart: ${g.pendingRestart?.type || 'none'}<br>camera: ${Math.round(g.camera.x)},${Math.round(g.camera.y)}<br>selected: ${g.selected ? g.selected.name : 'None'}<br>event: ${g.ui?.activeState || 'none'}<br>AI: ${g.aiDecision || 'idle'}<br>closest: ${closest}`;
